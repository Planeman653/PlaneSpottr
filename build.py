#!/usr/bin/env python3
"""
Build script for PlaneSpottr executable installer.
Creates a PyInstaller bundle with embedded API key prompt.
"""

import os
import sys
import shutil
import subprocess

# Configuration
BUILD_DIR = 'dist'
BUILD_SPEC = 'build.spec'
NAME = 'PlaneSpottr'
VERSION = '0.1.2'
ICON = 'icon.ico'  # Optional custom icon path

def get_pyinstaller_args():
    """Return PyInstaller arguments."""
    return [
        '--name', NAME,
        '--version-name', VERSION,
        '--clean',
        '--onefile',
        '--noconsole',
        '--windowed',
        '--windowed-version', '0.6.0',
        '--hidden-import', 'requests',
        '--hidden-import', 'pandas',
        '--hidden-import', 'numpy',
        '--hidden-import', 'PyQt6',
        '--hidden-import', 'dotenv',
        '--add-data', '.env.example;.',
    ]

def main():
    """Main build process."""
    print('='*60)
    print(f'PlaneSpottr Build - Version {VERSION}')
    print('='*60)
    print()

    # Check PyInstaller
    if not shutil.which('pyinstaller'):
        print('Error: PyInstaller not found.')
        print('Please install it first: pip install pyinstaller')
        sys.exit(1)

    # Check for existing dist folder
    if os.path.exists(BUILD_DIR):
        print(f'Cleaning existing {BUILD_DIR}/ folder...')
        shutil.rmtree(BUILD_DIR)

    # Build command
    print('Building executable...')
    print()

    try:
        subprocess.run([sys.executable, '-m', 'PyInstaller', '--name', NAME, '--version-name', VERSION, '--clean', '--onefile', '--noconsole', '--windowed', '--windowed-version', '0.6.0', '--add-data', '.env.example;.', 'main.py'], check=True, capture_output=False)
    except subprocess.CalledProcessError as e:
        print(f'Build failed: {e}')
        sys.exit(1)

    # Copy .env.example to dist folder for reference
    if os.path.exists('.env.example'):
        dest_env = os.path.join(BUILD_DIR, '.env.example')
        shutil.copy('.env.example', dest_env)
        print(f'Copied .env.example to {BUILD_DIR}/')

    print()
    print('='*60)
    print(f'Build complete! Executable created at:')
    print(f'  {BUILD_DIR}/{NAME}.exe')
    print('='*60)
    print()
    print('To install:')
    print('  1. Download PlaneSpottr.exe')
    print('  2. Extract/Install it')
    print('  3. First run will prompt for FlightAware API key')
    print('  4. Key is saved for future use')

if __name__ == '__main__':
    main()
